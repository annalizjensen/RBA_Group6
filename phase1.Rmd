...
#### Import & Helper Function to Avoid Duplicate Code
...
per_df_fn <- function(fn_to_run) {
  for(key in df_set$keys()){
    fn_to_run(df_set$get(key), key)
  }
}
...
list_var <- function(df, data_set_name) {
  ...
  cat("Dataset:", data_set_name, new_line, " Features:", col_list, new_line)
  cat("\n")
}
...
```{r}
print_dim <- function(df, data_set_name) {
  dim_df <- dim(df)
  cat("Data set", data_set_name,"has", dim_df[1], "rows and", 
      dim_df[2], "columns.", new_line)
}
...
per_df_fn(function(x, y) print(head(x)))

per_df_fn(function(x, y) print(tail(x)))
```

#### Missing Values

```{r}
how_many_na = function(df,data_set_name) {
  no_na <- length(which(is.na(df)))
  cat("In", data_set_name, "there are this many missing values:", no_na, new_line)
}

per_df_fn(how_many_na)
```

#### Missing Values Per Column

```{r}
sum_of_na = function(df, data_set_name) {
  per_col_na <- sapply(df, function(col) sum(is.na(col)))
  cat("In", data_set_name, "we are missing this many values per column:\n")
  cat(paste(as.list(paste(names(df), per_col_na, collapse = new_line))), new_line)
  cat("------", new_line)
}
