...
list_var <- function(df,data_set_name){
  ...
  cat(paste("Dataset ", data_set_name, " Features:",col_list,sep = "\n"))
}
...
print_dim <- function(df,data_set_name){
  dim_df <- dim(df)
  cat(paste("Data set ",data_set_name,"has ", dim_df[1] ," rows and " , dim_df[2] , " columns",sep=' '),sep='\n')
}
per_df_fn(print_dim)
```
...
how_many_na = function(df,data_set_name) {
  ...
  cat(paste("Missing values in ",data_set_name," dataset is ",no_na,sep=' '),sep = '\n')
}
...
sum_of_na = function(df,data_set_name) {
  per_col_na <- sapply(df, function(col) sum(is.na(col)))
    cat(paste(""),sep='\n')
    cat(paste("Missing values per column in ",data_set_name," dataset is ",per_col_na,sep=' '),sep = '\n')
}
